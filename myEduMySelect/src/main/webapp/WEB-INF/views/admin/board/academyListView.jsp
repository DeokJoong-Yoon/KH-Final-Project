...
<body>
	...
	<main id="main" class="main">
		...
		<form id="academyForm" name="academyForm">
			...
			<div class="pagetitle">
				...
				<nav>
					<ol class="breadcrumb">
						...
						<li class="breadcrumb-item active">academy list</li>
					</ol>
				</nav>
			</div>
			<!-- End Page Title -->
						...
						<div class="card">
							<div class="card-body">
								...
								<table class="table table-hover">
									<thead>
										<tr>
											<th scope="col">academyId</th>
											<th scope="col">academyName</th>
											<th scope="col">academyManagerName</th>
											<th scope="col">academyManagerPhone</th>
											<th scope="col">academyJoinDate</th>
										</tr>
									</thead>
									<tbody>
										<c:choose>
											<c:when test="${not empty academyAdminList}">
												<c:forEach var="academy" items="${academyAdminList}" varStatus="statusNumber">
														<tr data-num="${academy.academyId}">
															<th scope="row">${academy.academyId}</th>
															<th class="goDetail text-start">${academy.academyName}</th>
															<td class="goDetail text-start">${academy.academyManagerName}</td>
															<td class="goDetail text-start">${academy.academyManagerPhone}</td>
															<td class="goDetail text-start">${academy.academyJoinDate}</td>
														</tr>
												</c:forEach>
											</c:when>
											<c:otherwise>
	...
	<script>
      	$(function() {
      		...
      		let word="<c:out value='${academyAdminVO.keyword}' />";
      		let value ="";
      		if (word != "") {
      			$("#keyword").val("<c:out value='${academyAdminVO.keyword}' />");
      			$("#search").val("<c:out value='${academyAdminVO.search}' />");
      			
      			if ($("#search").val() != 'academy_id') {
      				if($("#search").val() == 'academy_name') value = "#list tr td.goDetail";
	...
	<script src="/resources/include/admin/js/academyListView.js"></script>
</body>
</html>