...
<body>
	...
	<main id="main" class="main">
		<form id="detailForm">
			<input type="hidden" id="commonCommentNo" name="commonCommentNo" />
		</form>
								...
								<div class="row g-2 align-items-center">
									...
									<div class="col-auto">
										<select id="search" name="search" class="form-select form-select-sm">
											...
											<option value="common_comment_content">Content</option>
										...
										<c:choose>
											<c:when test="${not empty commentList}">
												<c:forEach var="comment" items="${commentList}" varStatus="statusNumber">
														<tr data-num="${comment.commonCommentNo}">
															<th scope="row">${comment.commonCommentNo}</th>
															<th class="goDelete text-start">${comment.commonNo}</th>
															<td class="goDelete text-start">${comment.personalId}</td>
															<td class="goDelete text-start">${comment.commonCommentContent}</td>
															<td class="goDelete text-start">${comment.commonCommentDate}</td>
														</tr>
												</c:forEach>
											</c:when>
											<c:otherwise>
	...
	<script>
      	$(function() {
      		...
      		let word="<c:out value='${freeReplyAdminVO.keyword}' />";
      		let value ="";
      		if (word != "") {
      			$("#keyword").val("<c:out value='${freeReplyAdminVO.keyword}' />");
      			$("#search").val("<c:out value='${freeReplyAdminVO.search}' />");
      			
      			if ($("#search").val() != 'common_comment_content') {
	...
	<script src="/resources/include/admin/js/freeCommentListView.js"></script>
</body>
</html>